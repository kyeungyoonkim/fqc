...
def render_dashboard(df: pd.DataFrame, line_keys: list[str], targets: dict[str, float], title: str, out_path: Path) -> None:
    ...
    fig, axes = plt.subplots(rows, cols, figsize=(16, 4.5 * rows + 1.0))
    ...
    top_margin = 1.0 - (1.1 / fig.get_figheight())
    fig.tight_layout(rect=[0, 0, 1, top_margin])

    fig.suptitle(title, fontsize=16, fontweight="bold", y=top_margin + 0.065)
    handles, labels = flat_axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, top_margin + 0.015), ncol=4, frameon=False)
...
def main() -> None:
    ...
    parser.add_argument(
        "--auto-export", action="store_true", help="Run browser automation to download MES exports first (CTV, DLT)"
    )
    ...
    if args.auto_export:
        from mes_collector import collect_all_web_exports

        logging.info("Auto-export: downloading CTV/DLT MES files via browser")
        collect_all_web_exports(cfg, start, end)
